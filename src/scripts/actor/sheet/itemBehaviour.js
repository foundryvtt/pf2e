...
Hooks.on("preCreateOwnedItem", (actor, item) => {
  ...
  Actor.prototype.itemFeatDiehard = async function (create = true) {
    if (create) {
      ...
      await this.update({
        // # Refactor if there are also other methods that affect when you die from the Dying condition.
  ...
  Actor.prototype.itemFeatToughness = async function (create = true) {
    if (create) {
      ...
      const levelBonusHp = parseInt(getProperty(this.data.data.attributes, 'levelbonushp.value') || 0, 10); //parseInt can be removed once this value is integer safe (wrongly implemented as string)
      ...
      await this.update({
        ...
        'data.attributes.levelbonushp.value': levelBonusHp+1
      });
    } else {
      let recoveryMod = getProperty(this.data.data.attributes, 'dying.recoveryMod') || 0;
      const levelBonusHp = parseInt(getProperty(this.data.data.attributes, 'levelbonushp.value') || 1, 10); //parseInt can be removed once this value is integer safe (wrongly implemented as string)
