<script lang="ts">
    ...
    // const lores = $derived(data.lores);
    const proficiencyRanks = $derived(data.proficiencyRanks);
    const dcAdjustments = $derived(data.dcAdjustments);
    const partyLevel = $derived(data.partyLevel);
