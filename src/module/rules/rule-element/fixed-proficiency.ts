...
import { ATTRIBUTE_ABBREVIATIONS, SKILL_LONG_FORMS } from "@actor/values.ts";
...
class FixedProficiencyRuleElement extends RuleElementPF2e<FixedProficiencyRuleSchema> {
    ...
    override afterPrepareData(): void {
        const proficiency = this.resolveInjectedProperties(this.selector);
        const systemData = this.actor.system;
        const statistic = setHasElement(SKILL_LONG_FORMS, proficiency)
