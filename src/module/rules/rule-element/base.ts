...
import { NullableBooleanField, PredicateField, SlugField } from "@system/schema-data-fields.ts";
...
> {
    ...
    constructor(source: RuleElementSource, options: RuleElementOptions) {
        ...
        if (!validActorType) {
            const actorType = _loc(`TYPES.Actor.${item.actor.type}`);
            this.failValidation(`this rule element type cannot be applied to a ${actorType}`);
            source.ignored = true;
        }

        this.label = this.label
            ? _loc(this.resolveInjectedProperties(this.label), {
