...
export class Migration747FixedHeightening extends MigrationBase {
    ...
    override async updateItem(item: ItemSourcePF2e): Promise<void> {
        ...
        if (item.data.heightening?.type === "fixed") return;

        const sourceId = item.flags.core?.sourceId ?? "";
        if (fixedHeightenSpells.has(sourceId)) {
            const spell = await this.#getSpellCached(sourceId);
    ...
    async #getSpellCached(uuid: string) {
        if (objectHasKey(this.#loadedSpells, uuid)) {
            return this.#loadedSpells[uuid];
        }

        const spell = await fromUuid(uuid);
        if (spell instanceof SpellPF2e) {
            this.#loadedSpells[uuid] = spell;
            return spell;
        }
...
const fixedHeightenSpells = new Set([
