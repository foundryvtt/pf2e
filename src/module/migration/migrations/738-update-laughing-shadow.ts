...
export class Migration738UpdateLaughingShadow extends MigrationBase {
    ...
    #shadow = getCompendiumSource<ItemSourcePF2e>("Compendium.pf2e.classfeatures.3gVDqDPSz4fB5T9G");

    #cascade = getCompendiumSource<ItemSourcePF2e>("Compendium.pf2e.feature-effects.fsjO5oTKttsbpaKl");
    ...
    override async updateItem(source: ItemSourcePF2e): Promise<void> {
        if (source.type === "feat" && source.system.slug === "laughing-shadow") {
            const laughingShadow = this.#shadow;
            if (!laughingShadow) return;
            source.system.rules = laughingShadow.system.rules;
        } else if (source.type === "effect" && source.system.slug === "stance-arcane-cascade") {
            const arcaneCascade = this.#cascade;
            if (!arcaneCascade) return;

            const newRules = arcaneCascade.system.rules;
