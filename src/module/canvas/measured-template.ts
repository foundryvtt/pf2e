...
class MeasuredTemplatePF2e extends MeasuredTemplate<MeasuredTemplateDocumentPF2e> {
    ...
    override highlightGrid(): void {
        ...
        // Only highlight for objects that have a defined shape
        if (!(this.id && this.shape)) return;

        MeasuredTemplatePF2e.highlightGrid({
    ...
    static highlightGrid({ type, object, colors, data }: HighlightGridParams): void {
        ...
        const placeableName = object instanceof TokenPF2e ? "Token" : "Template";
        const highlightLayer = grid.getHighlightLayer(`${placeableName}.${object.id}`)?.clear();
        const { x, y } = data;
