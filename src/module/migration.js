...
/* -------------------------------------------- */
function createCoinItem(actor, denomination, quantity, coinData) {
    return Item.createOwned({
        'flags': {},
        'name': coinData.name,
        'type': 'currency',
        'data': {
            ...
            'stackGroup': { 'value': 'coins' },
            'denomitation': { 'value': denomination },
            'value': { 'value': 1 },
            'denomination': { 'value': denomination }
        },
        'sort': 100000,
        'img': `systems/pf2e/icons/equipment/held-items/${coinData.imagePartial}-pieces.jpg`
    }, actor);
}
...
function migrateCoins(actor) {
    ...
    if (ppQuantity !== null) {
        const pp = createCoinItem(actor,'pp', ppQuantity, coinData.pp);
        actor.items.push(pp);   
    }

    const gpQuantity = actor.data?.currency?.gp?.value ?? null;
    if (gpQuantity !== null) {
        const gp = createCoinItem(actor,'gp', gpQuantity, coinData.gp);
       actor.items.push(gp);
    }
    
    const spQuantity = actor.data?.currency?.sp?.value ?? null;
    if (spQuantity !== null) {
        const sp = createCoinItem(actor,'sp', spQuantity, coinData.sp);
       actor.items.push(sp);
    }
    
    const cpQuantity = actor.data?.currency?.cp?.value ?? null;
    if (cpQuantity !== null) {
        const cp = createCoinItem(actor,'cp', cpQuantity, coinData.cp);
       actor.items.push(cp);
    }
}
