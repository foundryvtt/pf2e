...
export class CharacterBuilder extends FormApplication {
    ...
    async _onDrop(event: DragEvent) {
        ...
        const containerId: keyof BuildCategories =
        ...
        // Add Item to Actor
        if (!dragItem.data) {
            // don't create another item since it comes from the Actor
    ...
    _onBuildItemDelete(event) {
        ...
        const containerId: keyof BuildCategories =
            target.data('containerId') ?? target.parents('[data-container-id]').data('containerId');
        this.build.choices[containerId].choices = this.build.choices[containerId].choices.filter(
            (x) => x.itemId !== itemId,
        );
