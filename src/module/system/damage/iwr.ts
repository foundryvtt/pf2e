...
function applyIWR(actor: ActorPF2e, roll: Rolled<DamageRoll>): IWRApplicationData {
    ...
    const applications = instances.flatMap((instance): IWRApplication[] => {
        ...
        if (objectHasKey(damageIsApplicable, instance.type) && !damageIsApplicable[instance.type]) {
            return [{ iwr: "inapplicable", type: instance.type, adjustment: -1 * total }];
        }

        // Step 2: Immunities
        const immuneToDamageType = immunities.some((i) => i.test(formalDescription));
...
interface InapplicableApplication {
    iwr: "inapplicable";
    type: string;
    adjustment: number;
}
...
type IWRApplication = InapplicableApplication | ImmunityApplication | WeaknessApplication | ResistanceApplication;

export { IWRApplicationData, applyIWR };
