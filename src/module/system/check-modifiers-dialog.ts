...
export class CheckModifiersDialog extends Application {
    ...
    static async roll(
        ...
        if (ctx.dc !== undefined) {
            ...
            flavor += `<div data-visibility="${ctx.dc.visibility ?? showResults}" class="degree-of-success"><b>${resultLabel}:`;
            flavor += `<span class="${degreeOfSuccessText}"> ${degreeLabel}</span></b>${adjustmentLabel}`;
            flavor += '</div>';
        }
