...
import type { CustomDamageData, ModuleHomebrewData } from "./data.ts";
...
class DamageTypeManager {
    #reservedTerms: ReservedTermsRecord;

    #moduleData: ModuleHomebrewData;
    ...
    constructor({ reservedTerms, moduleData }: { reservedTerms: ReservedTermsRecord; moduleData: ModuleHomebrewData }) {
    ...
    addCustomDamage(data: CustomDamageData, options: { slug?: string } = {}): void {
        ...
        // Exit out early if this is a reserved term
        if (this.#reservedTerms.damageTypes.has(slug)) {
            return;
        }
    ...
    updateSettings(): void {
        const reservedTerms = this.#reservedTerms;
        ...
        for (const [slug, data] of Object.entries(this.#moduleData.damageTypes)) {
