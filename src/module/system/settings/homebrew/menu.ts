...
import { SKILL_EXPANDED, SKILL_SLUGS } from "@actor/values.ts";
...
class HomebrewElements extends SettingsMenuPF2e {
    ...
    protected static override get settings(): Record<HomebrewKey, PartialSettingsData> {
        return {
            ...
            damageTypes: {
                ...
                type: Object,
                onChange: () => {
                    new DamageTypeManager({
                        reservedTerms: this.reservedTerms,
    ...
    onInit(): void {
        ...
        this.#refreshSettings();
        this.#registerModuleTags(moduleData);
        new DamageTypeManager({ reservedTerms: HomebrewElements.reservedTerms, moduleData }).updateSettings();

        for (const [slug, data] of Object.entries(moduleData.additionalSkills)) {
            SKILL_EXPANDED[slug as SkillSlug] = { attribute: data.attribute };
            SKILL_SLUGS.add(slug as SkillSlug);
            CONFIG.PF2E.skillList[slug as SkillSlug] = data.label;
        }
    }
    ...
    #registerModuleTags(settings: ModuleHomebrewData): void {
