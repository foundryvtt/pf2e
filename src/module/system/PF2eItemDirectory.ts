export default class extends ItemDirectory {
    ...
    initialize() {
        // Assign Folders
        this.folders = game.folders.filter(f => f.type === "Item");

        // Assign Entities
        this.entities = game.items.reduce((result, item) => {
        ...
        // Build Tree
        const sortMode = 'n';
        this.tree = SidebarDirectory.setupFolders(this.folders, this.entities, sortMode);
    }
