...
export class PF2EFamiliar extends PF2EActor<FamiliarData> {
    ...
    prepareData(): void {
        ...
        const rules = this.data.items.reduce((accumulated, current) => accumulated.concat(PF2RuleElements.fromOwnedItem(current)), []);
        ...
        if (master instanceof PF2ECharacter || master instanceof PF2ENPC) {
            ...
            for (let idx = 0; idx < data.attributes.speed.otherSpeeds.length; idx++) {
                ...
                const stat = mergeObject(
                    new PF2StatisticModifier(
                        game.i18n.format('PF2E.SpeedLabel', { type: speed.label }), modifiers
                    ),
                    speed, { overwrite: false }
                );
