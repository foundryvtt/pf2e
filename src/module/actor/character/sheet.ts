...
class CharacterSheetPF2e<TActor extends CharacterPF2e> extends CreatureSheetPF2e<TActor> {
    ...
    protected override activateClickListener(html: HTMLElement): SheetClickActionHandlers {
        ...
        // Rename a strike generated by a Strike rule element
        handlers["rename-strike"] = (_, anchor) => {
            const weapon = this.getStrikeFromDOM(anchor)?.item;
            const itemNameEl = anchor.previousElementSibling;
            if (!weapon?.flags.pf2e.renamableStrike || !(itemNameEl instanceof HTMLAnchorElement)) {
                throw ErrorPF2e("Unexpected error renaming strike");
            }

            const saveAnchor = createHTMLElement("a", {
                children: [fontAwesomeIcon("save")],
                dataset: { tooltip: "Save" },
            });

            // Remove body listener and restore sheet state
            const finish = (clickEvent?: MouseEvent) => {
                clickEvent?.stopImmediatePropagation();
                clickEvent?.preventDefault();
                renameInput.replaceWith(itemNameEl);
                saveAnchor.replaceWith(anchor);
                document.body.removeEventListener("click", finish);
            };

            // Update the RE label
            const save = (newLabel: string) => {
                itemNameEl.innerText = newLabel;
                finish();
                return weapon.rule?.relabel(newLabel);
            };

            saveAnchor.addEventListener("click", (event) => {
                event.stopPropagation();
                const newLabel = renameInput.value.trim();
                if (newLabel.length > 0) save(newLabel);
            });

            const renameInput = createHTMLElement("input", { classes: ["strike-label"] });
            renameInput.type = "text";
            renameInput.spellcheck = false;
            renameInput.value = weapon.name;
            renameInput.style.width = `${Math.floor(itemNameEl.clientWidth * 1.25)}px`;
            renameInput.addEventListener("keypress", (event): Promise<void> | void => {
                const newLabel = renameInput.value.trim();
                if (event.key === "Enter" && newLabel.length > 0) {
                    return save(newLabel);
                } else if (event.key === "Escape") {
                    finish();
                }
            });
            anchor.replaceWith(saveAnchor);
            itemNameEl.replaceWith(renameInput);
            renameInput.focus();
