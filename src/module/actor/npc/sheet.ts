...
export class NPCSheetPF2e extends CreatureSheetPF2e<NPCPF2e> {
    ...
    private prepareActions(actorData: NPCSheetData) {
        ...
        for (const action of this.actor.itemTypes.action) {
            const itemData = action.toObject(false);
            ...
            const traits = itemData.data.traits.value.map((traitString) => {
            ...
            if (objectHasKey(actions, actionType)) {
                actions[actionType].actions.push({
                    ...itemData,
                    glyph: getActionGlyph(action.actionCost),
                    imageUrl: getActionIcon(action.actionCost),
                    traits: traits.filter((p) => !!p),
                });
            }
        }
