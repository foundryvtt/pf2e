...
export abstract class CreatureSheetPF2e<ActorType extends CreaturePF2e> extends ActorSheetPF2e<ActorType> {
    ...
    activateListeners(html: JQuery): void {
        ...
        html.find('.spell-attack').on('click', (event) => {
            ...
            const id = $(event.currentTarget).closest('[data-container-id]').data('containerId');
            const entry = this.actor.data.items.find((item) => item._id === id) as SpellcastingEntryData;
