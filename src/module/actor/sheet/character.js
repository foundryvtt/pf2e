...
class ActorSheetPF2eCharacter extends ActorSheetPF2e {
  ...
  _prepareItems(actorData) {
    ...
    let totalWeight = 0;
    const bulkConfig = {coinBulk: game.settings.get('pf2e', 'coinBulk')};
    for (const i of actorData.items) {
      ...
      if (Object.keys(inventory).includes(i.type)) {
        ...
        const approximatedBulk = calculateBulk([toItemOrContainer(i)], stacks, bulkConfig);
    ...
    const bulk = calculateBulk(items, stacks, bulkConfig);
