...
export abstract class ActorSheetPF2e<TActor extends ActorPF2e> extends ActorSheet<TActor, ItemPF2e> {
        ...
        $html.find(".inventory .item .item-name").on("click", (event) => {
            ...
            if (sourceItemQuantity > 1) {
                const popup = new MoveLootPopup(
                    actor,
                    { maxQuantity: halfSourceItemQuantity, dontStack: true, lockStack: true },
                    (quantity, dontStack) => {
                        actor.transferItemToActor(actor, item, quantity, containerId, dontStack);
                    }
                );
                popup.render(true);
            }
    ...
    async moveItemBetweenActors(
        ...
        if (sourceItemQuantity > 1) {
            const popup = new MoveLootPopup(sourceActor, { maxQuantity: sourceItemQuantity }, (quantity, dontStack) => {
