import { SaveString, SkillString } from '@actor/actor-data-definitions';
...
export class ActorSheetPF2eFamiliar extends ActorSheet<PF2EFamiliar> {
    ...
    activateListeners(html: JQuery) {
        ...
        html.find('[data-skill-check]:not([data-skill-check=""])').on('click', '*', (event) => {
            const skill = $(event.currentTarget).closest('[data-skill-check]').attr('data-skill-check') as SkillString;
