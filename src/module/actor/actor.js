...
export default class extends Actor {
   ...
   */
  prepareData() {
    super.prepareData(); // Get the Actor's data object
    ...
    this._prepareTokenImg(); // Ability modifiers
    ...
    if (actorData.type === 'character') this._prepareCharacterData(data);else if (actorData.type === 'npc') this._prepareNPCData(data); // TODO: Migrate trait storage format
  ...
  _prepareCharacterData(data) {
    const character = new CharacterData(data, this.items); // Level, experience, and proficiency

    data.details.level.value = character.level;
    data.details.xp.max = character.maxExp;
    data.details.xp.pct = character.xpPercent; // Calculate HP and SP
    ...
    if (game.settings.get('pf2e', 'staminaVariant')) {
      ...
      const maxHp = ancestryHp + (classHp + conMod) * data.details.level.value + bonusHpPerLevel + flatBonusHp;
      data.attributes.hp.max = maxHp;
    } // Saves
    ...
    data.attributes.perception.breakdown = `${data.attributes.perception.ability} modifier(${data.abilities[data.attributes.perception.ability].mod}) + proficiency(${proficiency}) + item bonus(${data.attributes.perception.item})`; // TODO: seems like storing items, feats, armor, actions etc all in one array would be expensive to search? maybe adjust this data model?
    // TODO: speed penalties are not automated

    data.attributes.ac.value = character.ac;
    data.attributes.ac.check = character.skillCheckPenalty; // Skill modifiers
  ...
  _prepareNPCData(data) {} // As we only capture the NPCs Spell DC attribute, we need to calculate the Spell Attack Roll.
  ...
  rollSkill(event, skillName) {
    ...
    const parts = ['@mod'];
    const flavor = `${rank} ${CONFIG.PF2E.skills[skillName]} Skill Check`; // Call the roll helper utility
  ...
  rollRecovery(event) {
    ...
    const recoveryDc = 10 + recoveryMod; //rework later to also add Mountain's Stoutness support
  ...
  rollLoreSkill(event, item) {
    ...
    let rollMod = modifier + proficiency + itemBonus; // Override roll calculation if this is an NPC "lore" skill
  ...
  rollSave(event, saveName) {
    ...
    const parts = ['@mod'];
    const flavor = `${CONFIG.PF2E.saves[saveName]} Save Check`; // Call the roll helper utility
  ...
  rollAbility(event, abilityName) {
    ...
    const parts = ['@mod'];
    const flavor = `${CONFIG.PF2E.abilities[abilityName]} Check`; // Call the roll helper utility
  ...
  rollAttribute(event, attributeName) {
    ...
    const parts = ['@mod'];
    const flavor = `${game.i18n.localize("PF2E.PerceptionLabel")} Check`; // Call the roll helper utility
  ...
  async modifyTokenAttribute(attribute, value, isDelta = false, isBar = true) {
    ...
    if (attribute == 'attributes.hp') {
        ...
        if (value < 0) {
          ...
          if (game.settings.get('pf2e', 'staminaVariant') > 0 && value < 0) {
            ...
            if (currentSP.value + value >= 0) {
              ...
              this.update({
                [`data.attributes.sp.value`]: newSP
              });
              value = 0;
            } else {
              value = currentSP.value + value;
              this.update({
                [`data.attributes.sp.value`]: 0
              });
            }
          }
        }
      ...
      return this.update({
        [`data.attributes.hp.value`]: value
      });
      ...
      if (isDelta) {
        if (value < 0) {
          ...
          if (value < 0) {
            //substract the value from (temp)HP as well 
            if (hp.temp + value >= 0) {
      ...
      value = Math.clamped(value, 0, current.max);
      return this.update({
        [`data.attributes.shield.value`]: value
      });
    }
