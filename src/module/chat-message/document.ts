...
class ChatMessagePF2e extends ChatMessage<ActorPF2e> {
    ...
    get item(): Embedded<ItemUnionPF2e> | null {
        // If this is a strike, we usually want the strike's item
        const strike = this._strike;
        if (strike?.item) return strike.item as Embedded<ItemUnionPF2e>;
    ...
    getItemFromDOM(): Embedded<ItemUnionPF2e> | null {
        ...
        try {
            ...
            const item = itemSource
                ...
                : null;
            return item as Embedded<ItemUnionPF2e> | null;
        } catch (_error) {
            return null;
        }
    }
