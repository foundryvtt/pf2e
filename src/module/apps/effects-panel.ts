...
export class EffectsPanel extends Application {
    ...
    override activateListeners($html: JQuery): void {
        ...
        for (const effectEl of htmlQueryAll(html, ".effect-item[data-item-id]")) {
            ...
            iconElem?.addEventListener("click", async (event?: JQuery.TriggeredEvent | Event | null) => {
                const { actor } = this;
                ...
                if (actor && effect?.isOfType("condition") && effect.slug === "persistent-damage") {
                    ...
                    await effect.increase(event.shiftKey);
            ...
            iconElem?.addEventListener("contextmenu", async (event?: JQuery.TriggeredEvent | Event | null) => {
                const { actor } = this;
                const effect = actor?.items.get(itemId);
                if (effect instanceof AfflictionPF2e && isRelevantEvent(event)) {
                    await effect.decrease(event.shiftKey);
                } else if (effect instanceof AbstractEffectPF2e) {
