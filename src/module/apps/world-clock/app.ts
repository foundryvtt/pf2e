...
export class WorldClock extends fa.api.HandlebarsApplicationMixin(fa.api.ApplicationV2) {
    ...
    /** Input values retrieved prior to re-render and restored afterward. */
    #lastInputValues = { value: "", unit: "1" };
    ...
    /** Restore the input values for restoration after re-render. */
    protected override _preRender(context: object, options: fa.ApplicationRenderOptions): Promise<void> {
        if (this.rendered) {
            const valueInput = this.element.querySelector<HTMLInputElement>("input[name=diff-value]");
            const unitSelect = this.element.querySelector<HTMLSelectElement>("select[name=diff-unit]");
            this.#lastInputValues = { value: valueInput?.value ?? "", unit: unitSelect?.value ?? "1" };
        }
    ...
    protected override async _onRender(
        ...
        if (valueInput && unitSelect) {
            valueInput.value = this.#lastInputValues.value;
            unitSelect.value = this.#lastInputValues.unit;
        }
    }
    ...
    /** Temporarily disable the form, allowing for some time for the animation to complete. */
    #disableForm() {
        for (const button of this.element.querySelectorAll<HTMLButtonElement>(".window-content button")) {
            button.setAttribute("disabled", "");
            window.setTimeout(() => button.removeAttribute("disabled"), 2000);
        }
    }
