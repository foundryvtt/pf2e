...
class RunePageSheet extends fa.sheets.journal.JournalEntryPageHandlebarsSheet {
    ...
    async #prepareViewContext(): Promise<ViewRenderContext> {
        ...
        const variantPromises = fu
            .iterateEntries(page.system.variants)
            .map(async ([level, variant]) => {
                if (!variant) throw ErrorPF2e("Unexpected missing data");
                const addendum = await TextEditorPF2e.enrichHTML(variant.addendum);
                const price = variant.price.toString({ comma: true });
                return { ...variant, level: Number(level), price, addendum };
            })
            .toArray();
        const variants: RuneVariantSheetData[] = (await Promise.all(variantPromises)).sort((a, b) => a.level - b.level);
    ...
    async #prepareEditContext(): Promise<EditRenderContext> {
        ...
        const variants = this._prepareTabs("variants");
        ...
        return {
            ...
            variants,
            canDeleteVariants: Object.keys(variants).length > 1,
            priceUnit,
