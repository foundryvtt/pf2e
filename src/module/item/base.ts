...
export class ItemPF2e extends Item<ActorPF2e, ActiveEffectPF2e> {
    ...
    /**
     * Called when a click event on an item card's button has occured.
     */
    async handleButtonEvent(this: Owned<ItemPF2e>, ev: JQuery.ClickEvent) {
        const button = $(ev.currentTarget);
        const action = button.attr('data-action');
        const actor = this.actor;

        const strike: StatisticModifier = actor.data.data?.actions?.find((a: StatisticModifier) => a.item === this.id);
        ...
        if (action === 'weaponAttack') {
            if (strike && rollOptions) {
                ...
                this.rollWeaponAttack(ev);
            }
        } else if (action === 'weaponAttack2') {
            if (strike && rollOptions) {
                strike.variants[1].roll({ event: ev, options: rollOptions });
            } else {
                this.rollWeaponAttack(ev, 2);
            }
        } else if (action === 'weaponAttack3') {
            if (strike && rollOptions) {
                strike.variants[2].roll({ event: ev, options: rollOptions });
            } else {
                this.rollWeaponAttack(ev, 3);
            }
        } else if (action === 'weaponDamage') {
            if (strike && rollOptions) {
                strike.damage({ event: ev, options: rollOptions });
            } else {
                this.rollWeaponDamage(ev);
            }
        } else if (action === 'weaponDamageCritical' || action === 'criticalDamage') {
            if (strike && rollOptions) {
                strike.critical({ event: ev, options: rollOptions });
            } else {
                this.rollWeaponDamage(ev, true);
            }
        } else if (action === 'npcAttack') this.rollNPCAttack(ev);
        else if (action === 'npcAttack2') this.rollNPCAttack(ev, 2);
        else if (action === 'npcAttack3') this.rollNPCAttack(ev, 3);
        else if (action === 'npcDamage') this.rollNPCDamage(ev);
        else if (action === 'npcDamageCritical') this.rollNPCDamage(ev, true);
        // Spell actions
        else if (action === 'spellAttack') this.rollSpellAttack(ev);
        else if (action === 'spellAttack2') this.rollSpellAttack(ev, 2);
        else if (action === 'spellAttack3') this.rollSpellAttack(ev, 3);
        else if (action === 'spellDamage') this.rollSpellDamage(ev);
        else if (action === 'spellCounteract') this.rollCounteract(ev);
        // Consumable usage
        else if (action === 'consume') this.rollConsumable(ev);
        else if (action === 'save') ActorPF2e.rollSave(ev, this);
    }
