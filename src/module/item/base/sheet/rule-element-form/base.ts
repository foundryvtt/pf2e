...
import type { RuleElementSchema, RuleElementSource } from "@module/rules/rule-element/data.ts";
...
> {
    ...
    protected getInitialValue({ autogenerate = false } = {}): object {
        ...
        const initial: Partial<fields.SourceFromSchema<RuleElementSchema>> | undefined = this.schema?.getInitialValue();
        if (!initial) return {};
    ...
    /** Update a rule-elements array following changes made to this rule element. */
    async updateItem(updates: Partial<TSource> | Record<string, JSONValue>): Promise<void> {
        ...
        if (this.schema) {
            this.schema.clean(result, { copy: false, prune: false });
            this.#pruneInitialValues(this.schema.fields, result);
        }
    ...
    updateObject(source: TSource & Partial<Record<string, JSONValue>>): void {
        ...
        if (this.schema) {
            this.schema.clean(source, { copy: false, prune: false });
            this.#pruneInitialValues(this.schema.fields, source);
        }
    ...
    /** Recursively remove all fields that have a default value. */
    #pruneInitialValues(schema: DataSchema, data: Record<string, unknown>, _depth = 0): void {
        for (const [key, field] of fu.iterateEntries(schema)) {
            if (this.#pruneInitialValue(data, key, field)) continue;

            if ("fields" in field) {
                const value = data[key];
                if (R.isPlainObject(value)) {
                    this.#pruneInitialValues(field.fields as DataSchema, value, _depth + 1);
                    this.#pruneInitialValue(data, key, field);
                    continue;
                }
            }
            ...
            if (field instanceof fields.ArrayField && field.element instanceof fields.SchemaField) {
                ...
                if (Array.isArray(value)) {
                    ...
                    for (const element of value) {
                        if (R.isPlainObject(element)) {
                            if (element.predicate) this.#cleanPredicate(element);
                            this.#pruneInitialValues(field.element.fields, element, _depth + 1);
                        }
                    }
                    continue;
                }
    ...
    /**
     * Prune a single initial value from a proposed rule element update
     * @returns Whether the value was pruned
     */
    #pruneInitialValue(data: Record<string, unknown>, key: string, field: fields.DataField<unknown, unknown>): boolean {
        const value = data[key];
        if (value === undefined) {
            delete data[key];
            return true;
        }
        if ("choices" in field && field.choices) return false;
        if (typeof field.initial === "function" && field.name !== "priority") return false;
        const initialValue = field.getInitialValue(data);
        const mergedValue =
            R.isPlainObject(value) && R.isPlainObject(initialValue) ? { ...initialValue, ...value } : value;
        if (R.isDeepEqual(initialValue, mergedValue)) delete data[key];
        return !(key in data);
    }
