...
function updateManifest(zipUrl) {
	...
	if (!config)
		throw Error(chalk.red('foundryconfig.json not found'));
	if (!manifest)
		throw Error(chalk.red('Manifest JSON not found'));
	if (!rawURL || !repoURL)
...
async function releaseAndTag(zipFile) {
  ...
  let file = fs.createReadStream(`package/${zipFile}`)
  form.append("file", file, zipFile)
  console.log("Uploading to Gitlab")
  let response = await ky.post(`https://gitlab.com/api/v4/projects/${gitlabId}/uploads`,
     {
      body: form,
      headers: {
        'PRIVATE-TOKEN': token
